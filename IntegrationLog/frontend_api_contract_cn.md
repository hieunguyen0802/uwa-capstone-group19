...
| 状态 | Academic 是否 confirmed | HoD 是否可见 | HoD 是否可操作 |
| --- | --- | --- | --- |
| `initial` | 否 | **不可见** | — |
| `initial` | 是 | **可见（只读）** | 不可操作 |
| `pending` | — | 可见 | 可 approve / reject |
| `approved` | — | 可见 | 不可操作（终态） |
| `rejected` | — | 可见 | 不可操作（终态） |

**状态转换：**
- `initial`（未 confirmed）→ Academic 点击 Confirm → `initial`（已 confirmed，HoD 可见）
- `initial`（已 confirmed）→ Academic 点击 Submit Request → `pending`
- `pending` → HoD Approve → `approved`
- `pending` → HoD Reject → `rejected`
...
HoD 页面的可见性由两个条件共同决定：**状态** 和 **Academic 是否已 confirmed**。

```
[Admin 派发] → initial（Academic 未 confirmed → HoD 不可见）
                    ↓ Academic 点击 Confirm
              initial（Academic 已 confirmed → HoD 可见，只读）
                    ↓ Academic 点击 Submit Request
              pending（HoD 可见，可 approve / reject）
                    ↓ HoD 操作
         approved / rejected（终态，HoD 可见，不可操作）
```

- **`initial` + 未 confirmed**：HoD 完全看不到这条记录
- **`initial` + 已 confirmed**：HoD 可以看到，但不能操作（Academic 还没有提交审批请求）
- **`pending`**：Academic 已提交，HoD 可以 approve 或 reject
- **`approved` / `rejected`**：终态，HoD 可查看历史
...
> **可见性规则**：HoD 只能看到以下两类记录：
> 1. `initial` 且 Academic **已 confirmed**（只读，不可操作）
> 2. `pending` / `approved` / `rejected`（Academic 已提交审批请求）
>
> `initial` 且 Academic **未 confirmed** 的记录对 HoD 完全不可见。
...
> `status=initial` 返回 Academic 已 confirmed 但尚未提交审批请求的记录（HoD 只读）。
